    ...
    if(output->id < digital.out.n_ports) {
        ...
        if(config->open_drain != aux_out[output->id].mode.open_drain) {
            //if((aux_out[output->id].mode.open_drain = config->open_drain))
                ...
                //gpio_init((uint32_t)aux_out[output->id].port, GPIO_MODE_OUT_OD, GPIO_OSPEED_50MHZ, aux_out[output->id].pin);
            //else
                //aux_out[output->id].port->OTYPER &= ~(GPIO_OTYPER_OT_0 << aux_out[output->id].pin);
                //gpio_init((uint32_t)aux_out[output->id].port, GPIO_MODE_OUT_PP, GPIO_OSPEED_50MHZ, aux_out[output->id].pin);
        
        uint32_t tmpctl = aux_out[output->id].pin>7 ? aux_out[output->id].port->CTL1 : aux_out[output->id].port->CTL0;
        tmpctl &= ~(0b1111<<((aux_out[output->id].pin&7)<<2));
        tmpctl |= (0b0011|(aux_out[output->id].mode.open_drain = config->open_drain<<2))<<((aux_out[output->id].pin&7)<<2);
        if (aux_out[output->id].pin>7) aux_out[output->id].port->CTL1 = tmpctl;
         else aux_out[output->id].port->CTL0 = tmpctl;
        }
...
static bool digital_in_cfg (xbar_t *input, gpio_in_config_t *config, bool persistent)
{
    if(input->id < digital.in.n_ports && config->pull_mode != PullMode_UpDown) {
        //uint8_t pullmode = GPIO_MODE_IN_FLOATING;
        ...
        //gpio_init((uint32_t)aux_in[input->id].port, pullmode, GPIO_OSPEED_50MHZ, input->pin);
        uint32_t tmpctl = input->pin>7 ? aux_in[input->id].port->CTL1 : aux_in[input->id].port->CTL0;
        tmpctl &= ~(0b1111<<((input->pin&7)<<2)); // bitmask
        tmpctl |= (config->pull_mode == PullMode_None ? 4 : 8)<<((input->pin&7)<<2); // 0b 01=nopull/10=pull 00=in_mode 
        if (input->pin>7) aux_in[input->id].port->CTL1 = tmpctl;
         else aux_in[input->id].port->CTL0 = tmpctl;
        if (config->pull_mode == PullMode_Up) {aux_in[input->id].port->OCTL |= 1<<input->pin;}
        else {aux_in[input->id].port->OCTL &= ~1<<input->pin;}
