...
{
    bool ok = hal.nvs.type != NVS_None;
    ok = ok && SETTINGS_VERSION == hal.nvs.get_byte(0);
    ok = ok && hal.nvs.memcpy_from_nvs((uint8_t *)&settings, NVS_ADDR_GLOBAL, sizeof(settings_t), true) == NVS_TransferResult_OK;
    sanity_check();
